...
class ExportMixin:
    ...
    def get_export_data(
        ...
        with transaction.atomic():
            ...
            tasks = list(tasks)
            logger.debug('Annotations filtration')
            ...
            logger.debug('Tasks filtration on empty annotations')
            ...
            counters['task_number'] = len(tasks)
    ...
    def convert_file(self, to_format):
        with get_temp_dir() as tmp_dir:
            ...
            if len(files) == 0:
                ...
                filename = pathlib.Path(input_name).stem + pathlib.Path(output_file).suffix + '.zip'
